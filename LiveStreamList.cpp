...
#include "ui_LiveStreamList.h"
#include<QMessageBox>
#include"RoomManager.h"
LiveStreamList::LiveStreamList(ClientSocket *cs, QTcpSocket *socket, user_t user, QWidget *parent) :
    cs(cs), socket(socket),
    user(user), QDialog(parent),
    ui(new Ui::LiveStreamList)
{
    ui->setupUi(this);
...
返回值：无
*/
void LiveStreamList::showRoomName()
{
    user.type = ROOMNAME;
    qDebug()<<"进入房间列表";
...
返回值：无
*/
void LiveStreamList::closeEvent (QCloseEvent *)
{
    user.type = EXIT;
...
返回值：无
*/
void LiveStreamList::onsigRoomName(user_t user)
{
...
返回值：无
*/
void LiveStreamList::on_chatroom_itemDoubleClicked(QListWidgetItem *item)
{
    ...
    str += qitem;
    strncpy(user.roomName, master,20);
    chatroom = new LiveStream(cs, socket, qitem, user, this);
...
返回值：无
*/
void LiveStreamList::on_destory_clicked()
{
...
返回值：无
*/
void LiveStreamList::on_exit_btn_clicked()
{
    QMessageBox::StandardButton sb = QMessageBox::information (this, "提示",
...
返回值：无
*/
void LiveStreamList::on_roomlist_btn_clicked()
{
    ui->chatroom->clear ();
    user.type = ROOMLIST;
    socket->write((char*)&user, sizeof(user));
...
返回值：无
*/
void LiveStreamList::onsigRoomList(user_t user)
{
