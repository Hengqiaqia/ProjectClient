...
LiveStream::LiveStream(ClientSocket *cs, QTcpSocket *socket, QString curRoom,
                       user_t user, QWidget *parent) :
    QDialog(parent),cs(cs),socket(socket),curRoom(curRoom),user(user),
    ui(new Ui::LiveStream)
{
    ui->setupUi(this);
...
返回值：bool
*/
bool LiveStream::eventFilter(QObject *obj, QEvent *ev)
{
    if(obj == ui->textEdit){
            //是否是键盘事件
            if(ev->type() == QEvent::KeyPress){
                ...
                if(keyEvent->key() == Qt::Key_Return){
                    LiveStream::on_send_clicked();    //执行发送按钮事件
                }else{
                    //继续转发
                    return false;
                }
            ...
            return LiveStream::eventFilter(obj, ev);
        }
}
...
返回值：无
*/
void LiveStream::uiDesign()
{
    //设置窗口背景图片
    this->setStyleSheet ("background-image:url(:/new/prefix1/client_bg.jpg)");
    ui->listWidget->setStyleSheet ("background-image:url(:/new/prefix1/clientrecord_bg.jpg)");
    ui->textBrowser->setStyleSheet ("background-image:url(:/new/prefix1/clientrecord_bg.jpg)");
    ui->textEdit->setStyleSheet ("background-image:url(:/new/prefix1/clientrecord_bg.jpg)");

   //设置按钮图片和无边框
    ui->send->setStyleSheet("QPushButton{background: transparent;background-color:blue;"
                            "color: black;"
                            "border-radius: 10px;"
    ...
    ui->video->setStyleSheet("QPushButton{background:transparent;}");

    QPixmap video(":/image/image/icon/video.png");
    QPixmap audio(":/new/prefix1/voice.png");
    QPixmap flash(":/new/prefix1/refresh.jpg");
    ...
    QPixmap pixmap(":/new/prefix1/video.png");
...
void LiveStream::udpVideo()
{
    ...
    groupAddress.setAddress ("192.168.11.105");//"224.0.1.200");
...
返回值：无
*/
void LiveStream::udpAudio()
{
    socketWdaudio = new QUdpSocket(this);
    socketRdaudio = new QUdpSocket(this);
    groupAddress1.setAddress ("192.168.11.105");//"224.0.1.205");
...
返回值：无
*/
void LiveStream::onsigChatName (user_t user)
{
...
返回值：无
*/
void LiveStream::on_chatlist_btn_clicked()
{
    ui->listWidget->clear ();
    user.type = CHATLIST;
...
返回值：无
*/
void LiveStream::onsigChatList (user_t user)
{
...
返回值：无
*/
void LiveStream::on_send_clicked()    //文字聊天
{
...
返回值：无
*/
void LiveStream::onsigChatText(user_t user)
{
...
返回值：无
*/
void LiveStream::closeEvent (QCloseEvent *)
{
...
返回值：无
*/
void LiveStream::onsigMasterExit(user_t user)
{
...
返回值：无
*/
void LiveStream::on_video_clicked()
{
    if(user.flag != MASTER){
    ...
    if(fv == -1){
        ...
        QPixmap video(":/new/prefix1/AIOBubbleIcon_Video_red.png");
        ui->video->setIcon (video);
        ui->video->setIconSize (QSize(35,35));
        qDebug()<<"开始直播user.type:"<<user.type;
        socket->write((char*)&user, sizeof(user));
        fv *= -1;
    }
    else if(fv == 1){        //关闭视频
        user.type = UNVIDEO;
        QPixmap video(":/new/prefix1/AIOBubbleIcon_Video.png");
        ui->video->setIcon (video);
        ui->video->setIconSize (QSize(35,35));
        socket->write((char*)&user, sizeof(user));
        fv *= -1;
...
返回值：无
*/
void LiveStream::onsigVideo (user_t user)
{
    if(user.flag != MASTER){
...
返回值：无
*/
void LiveStream::onsigunVideo (user_t user)
{
    if(user.flag != MASTER){
    ...
    QPixmap video(":/new/prefix1/videop.png");
    ui->video->setIcon (video);
    ui->video->setIconSize (QSize(35,35));
}
...
返回值：无
*/
void LiveStream::onvideoChanged(QVideoFrame curFrame)
{
    static int i = 0;
    i++;
...
返回值：无
*/
void LiveStream::on_bs_clicked()
{
    static int bs = -1;
    if(bs == -1){
        ui->textscroll->show ();
        QPixmap send(":/new/prefix1/roseimg.jpg");
        ui->bs->setIcon (send);
        ui->bs->setIconSize (QSize(35,35));
        bs *= -1;
    }
...
返回值：无
*/
void LiveStream::onsigBs (user_t user)
{
...
返回值：无
*/
void LiveStream::on_audio_clicked()
{
    static int af = -1;
    if(af == -1){
        QPixmap audio(":/new/prefix1/voice.png");
...
返回值：qint64
*/
qint64 LiveStream::addWavHeader(QString catheFileName , QString wavFileName)
{
    // 开始设置WAV的文件头
...
返回值：无
*/
void LiveStream::sendMsg(QString audioname)
{
...
返回值：无
*/
void LiveStream::ReadyReadAudio()
{
...
返回值：无
*/
void LiveStream::on_gift_clicked()
{
